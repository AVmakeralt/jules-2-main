    ...
    match name {
        "path::astar" => {
            ...
            if let (Value::Array(grid_arr), Some(w), Some(h), Some(sx), Some(sy), Some(gx), Some(gy)) =
                ...
                match path {
                    Some(p) => {
                        let vals: Vec<Value> = p.into_iter().map(|(x,y)| Value::Tuple(Box::new(vec![Value::I32(x), Value::I32(y)]))).collect();
                        Some(Ok(Value::Array(std::rc::Rc::new(std::cell::RefCell::new(vals)))))
                    }
                    None => Some(Ok(Value::Array(std::rc::Rc::new(std::cell::RefCell::new(vec![]))))),
                }
        ...
        "path::flow_field" => {
            ...
            if let (Value::Array(grid_arr), Some(w), Some(h), Some(gx), Some(gy)) =
                ...
                let vals: Vec<Value> = flow.into_iter().map(|[dx,dy]| Value::Tuple(Box::new(vec![Value::F32(dx), Value::F32(dy)]))).collect();
                Some(Ok(Value::Array(std::rc::Rc::new(std::cell::RefCell::new(vals)))))
